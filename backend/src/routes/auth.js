...
router.post("/login", async (req, res) => {
    ...
    if (!user) {
        return res.status(403).json({ message: "Failed to Login: This username doesn't exist."});
    }
    const match = await bcrypt.compare(password, user.hash);
    if (!match) {
        return res.status(403).json({ message: "Failed to Login: Wrong password."});
    }
...
router.post("/signup", async (req, res) => {
    ...
    if (!name || !surname || !username || !password) {
        return res.status(409).json({ message: "Failed to Signup: Invalid credentials, choose other ones."});
     }
